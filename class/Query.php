...
class Query implements InterfaceQuery
{
    ...
    */
    ...
    private function throwExpectionIfTableDontExist(string $table)
    {
        if (!$this->doTableExist($table)) {
            throw new QueryExpection("Query Error: Table '$table' does not exist");
        }
    }
        ...
        if ($cols) {
            foreach ($cols as $aCol) {
                if (!in_array($aCol, $this->getColumnsInTable($table))) {
                    throw new QueryExpection("Query Error: Column '$aCol' does not exist in Table '$table'");
                }
            }
        }
    }
    ...
    */
    ...
    {
        $length = 0;
        $operators = ['=', '>', '<', '>=', '<=', '<>', 'IS NULL', 'IS NOT NULL', 'LIKE', 'NOT LIKE'];
        foreach ($where as $key => $value) {
            if (!in_array($key, $this->getColumnsInTable($table))) {
                throw new DbConnectExpection("Query Error: invalid column in where parameter");
            }
            if (!in_array(trim(strtoupper($value[0])), $operators)) {
                throw new DbConnectExpection("Query Error: unsupported operator in where parameter");
            }
            if (!in_array(trim($value[2]), ['isValue', 'isFunction'])) {
                throw new DbConnectExpection("Query Error: indicate value as either 'isValue' or 'isFunction'");
            }
            ++$length;
            if ($length != count($where) && count($value) < 4) {
                throw new DbConnectExpection("Query Error: non-last index in the where parameter is missing join");
            }
            if (isset($value[3]) && (trim(strtolower($value[3])) != 'and' && trim(strtolower($value[3])) != 'or')) {
                throw new DbConnectExpection("Query Error: join in non-last index in the where parameter must be either 'and' or 'or'");
            }
        }
    ...
    private function pdoPrepareExecute(string $sql, array $params = null):array
    {
        ...
        try {
            if (!$PdoStatment = $this->dbConnect->prepare($sql)) {
                throw new QueryExpection("Query Error: SQL preparation failed '$sql'");
            }
            if ($params) {
            ...
            if (!$PdoStatment->execute()) {
                throw new QueryExpection("Query Error: {$PdoStatment->errorInfo()[2]}");
            }
            $rows = $PdoStatment->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            throw new QueryExpection("Query Error: {$e->getMessage()}");
        }
    ...
    */
    public function createTable(string $table, array $tableStructure)
    {
        ...
        $tableStructure = [
            "id" => [
                "index"=> "pk", "type"=> "int unsighted", "null"=> "no", "enum_val"=> [], "default"=> "",
        ...
        */
        if (in_array($table, $this->getTablesInDb())) {
            throw new QueryExpection("Query Error: Table '$table' already exist");
        }
        ...
        foreach ($tableStructure as $colName => $properties) {
            ...
            foreach ($properties as $propertyKey => $propertyValue) {
                if ($propertyKey == "index" || trim($propertyValue) == "pk") {
                    $keys .= " PRIMARY KEY ($colName), ";
                }
                if ($propertyKey == "index" || trim($propertyValue) != "pk") {
                    foreach (explode(",", trim($propertyValue)) as $anIndex) {
                ...
                if ($propertyKey == "type") {
                    $null = trim(strtolower($properties['type'])) == 'no' ? "NOT NULL" : "NULL";
                    $auto = trim(strtolower($properties['others'])) == 'auto no' ? "AUTO_INCREMENT" : "";
                    $property .= "  $colName {$properties['type']} $null $auto , ";
                }
            }
            $columns .= " $colName $property , ";
        }
        ...
        $sql .= " CREATE TABLE IF NOT EXISTS $table (
            $columns        
            $keys
        ) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8_general_ci;
        $alterStatement;";
        $sql .= "COMMIT;";
        $this->executeSql($sql);
    }
